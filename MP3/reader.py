...
def load_dataset(train_dir, dev_dir, stemming, lower_case):
    X0 = loadDir(train_dir + '/pos/',stemming, lower_case)    # list of lists of words from positive review documents
    X1 = loadDir(train_dir + '/neg/',stemming, lower_case)    # list of lists of words from negative review documents
    X = X0 + X1                                               # list of lists of words from all documents in training set
    Y = len(X0) * [1] + len(X1) * [0]
    Y = np.array(Y)                                           # list of binary labels
